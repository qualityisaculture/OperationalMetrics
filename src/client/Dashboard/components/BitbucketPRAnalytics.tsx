...
const BitbucketPRAnalytics = forwardRef<
  BitbucketPRAnalyticsRef,
  BitbucketPRAnalyticsProps
>(
  (
    {
    ...
    useEffect(() => {
      if (
        ...
        loadPullRequestsForAllRepositories(hasLoadedPRs)
          .then(() => {
    ...
    // Filter PRs by selected month for modal display
    const selectedMonthPRs = useMemo(() => {
      if (selectedMonth) {
        ...
        >
          {selectedMonth && (
            <div style={{ marginBottom: "16px" }}>
              <Text>
                Showing {selectedMonthPRs.length} pull request
                {selectedMonthPRs.length !== 1 ? "s" : ""}
                {` for ${prsByMonth.find((m) => m.monthKey === selectedMonth)?.month || selectedMonth}`}
              </Text>
            </div>
          )}
          <Table
            dataSource={selectedMonthPRs}
            columns={prColumns}
            rowKey={(record, index) => `${record.id || index}`}
            pagination={{ pageSize: 50 }}
            scroll={{ x: true }}
